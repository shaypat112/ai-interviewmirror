...
export function useConfidenceScore() {
  ...
  const [finalScore, setFinalScore] = useState<ConfidenceBreakdown | null>(
    null,
  );
  ...
  const analyzeFrame = useCallback(
    async (videoEl: HTMLVideoElement): Promise<ConfidenceBreakdown | null> => {
      if (!detectorRef.current || videoEl.readyState < 2) return null;

      if (videoEl.videoHeight === 0 || videoEl.videoWidth === 0) return null;
      ...
      try {
        ...
        const sizeScore =
          normW > 0.1 && normW < 0.6
            ? 100
            : Math.max(0, 100 - Math.abs(normW - 0.3) * 300);
        ...
        const overall = Math.round(
          presence * 0.35 + eyeContact * 0.45 + stability * 0.2,
        );
